...
public strictfp class RobotPlayer {
    ...
    static ArrayList<MapLocation> targets = new ArrayList<MapLocation>();
    /**
	        ...
	        if(turnCount%6==0) {
	        	...
	        	for(int i = 0; i < muckIDs.size(); i++) {
	        		if(rc.canGetFlag(muckIDs.get(i))) {
	        			...
	        			if(flag != 0) {
                            MapLocation flagloc = getLocationFromFlag(flag);
                            boolean a = targets.contains(flagloc);
                            if(!a) {
                                targets.add(flagloc);
                            } else if(a && getExtraFromFlag(flag) == 1){
	        				    targets.remove(flagloc);
                            }
	        			}
	        		} else {
	        			muckIDs.remove(i);
    ...
    static void runPolitician() throws GameActionException {
    	...
    	if(rc.getInfluence() > 50) {
            ...
            for(RobotInfo robot : robots) {
                if(robot.type == spawnableRobot[2]) {
                    if(robot.location.isWithinDistanceSquared(loc, actionRadius) && rc.canEmpower(actionRadius)) {
                        if((rc.getInfluence()-10)/rc.senseNearbyRobots().length !=0) {
                            rc.empower(actionRadius);
                            return;
                        } else {
                            if(robot.location.isWithinDistanceSquared(loc, 1) && rc.canEmpower(1)){
                                if((rc.getInfluence()-10)/rc.senseNearbyRobots(1).length !=0) {
                                    rc.empower(actionRadius);
                                    return;
                                }
                            }
    ...
    static void runMuckraker() throws GameActionException {
    	...
    	if(rc.canGetFlag(ecid)) {
    		if(rc.getFlag(ecid) != 0) {
    		    int extry = getExtraFromFlag(rc.getFlag(ecid));
